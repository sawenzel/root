...
inline std::array<double, 3> P(double x, double y, double z) { return {x, y, z}; }
...
inline std::array<double, 3> NormalAt(const TGeoShape &s,
                                      const std::array<double, 3> &p,
                                      const std::array<double, 3> &dir_hint)
{
...
// VecGeom DistanceToOut(p,v) ~ ROOT/TGeo DistFromInside(p,v,...)
inline double DistanceToOut(const TGeoShape &s,
                            const std::array<double, 3> &p,
                            const std::array<double, 3> &v,
                            int iact = 3)
{
  return s.DistFromInside(p.data(), v.data(), iact, /*step=*/0.0, /*safe=*/nullptr);
}

// VecGeom DistanceToIn(p,v) ~ ROOT/TGeo DistFromOutside(p,v,...)
inline double DistanceToIn(const TGeoShape &s,
                           const std::array<double, 3> &p,
                           const std::array<double, 3> &v,
                           int iact = 3)
{
  return s.DistFromOutside(p.data(), v.data(), iact, /*step=*/0.0, /*safe=*/nullptr);
}
...
TGeoTessellated *CreateTrdLikeTessellated_Triangles(const char *name,
                                                    double x1, double x2,
                                                    double y1, double y2,
                                                    double z)
{
...
// =============================================================================
// Tests (TGeo-style: Contains() boolean, no kSurface assertions, no SafetyToIn/Out)
// =============================================================================

TEST(TGeoTessellated, TrdLike_CoreNavigation_TGeoStyle)
{
  ...
  std::unique_ptr<TGeoTessellated> tsl3(CreateTrdLikeTessellated_Triangles(
      "BABAR Trd",
  ...
  // ---------------------------------------------------------------------------
  // Volume + Surface area (still meaningful if your implementation provides them)
  // ---------------------------------------------------------------------------
  EXPECT_NEAR(tsl1->Capacity(), 8.0 * 20.0 * 30.0 * 40.0, 1e-9);
  EXPECT_NEAR(tsl1->SurfaceArea(), 20800.0, 1e-9);

  // ---------------------------------------------------------------------------
  // "Inside" checks using TGeo convention: Contains(p) -> bool
  // (we drop all former kSurface expectations)
  // ---------------------------------------------------------------------------
  ...
  // For tsl2: same expected tilted side normals as your VecGeom test
  ...
  const double sina = 1.0 / std::sqrt(17.0);
  {
    ...
    // If your tessellated navigation uses the actual facets, these should be on the side surfaces.
    const auto px = P(20, 0, 0);
    const auto nx = P(-20, 0, 0);
    const auto py = P(0, 30, 0);
    const auto ny = P(0, -30, 0);
  ...
  // (we removed "on-surface => 0" checks because we no longer classify surface)
  // ---------------------------------------------------------------------------
  {
  ...
  // (kept from your original expectations)
  // ---------------------------------------------------------------------------
  {
